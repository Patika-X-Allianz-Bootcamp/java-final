...
public class SeatMapperImpl implements SeatMapper {
    ...
    public SeatEntity requestDtoToEntity(SeatRequestDTO requestDTO) {
        ...
        seatEntity.setSeatNo( requestDTO.getSeatNo() );
        seatEntity.setPatient( patientRequestDTOToPatientEntity( requestDTO.getPatient() ) );

        return seatEntity;
    }

    @Override
    public SeatResponseDTO entityToResponseDto(SeatEntity entity) {
        if ( entity == null ) {
            return null;
        }
        ...
        seatResponseDTO.setUuid( entity.getUuid() );
        seatResponseDTO.setCreationDate( entity.getCreationDate() );
        seatResponseDTO.setLastModifiedDate( entity.getLastModifiedDate() );
        seatResponseDTO.setSeatNo( entity.getSeatNo() );
        seatResponseDTO.setAvailable( entity.isAvailable() );
        seatResponseDTO.setPatient( patientEntityToPatientResponseDTO( entity.getPatient() ) );

        return seatResponseDTO;
    }

    @Override
    public List<SeatResponseDTO> entityListToResponseDtoList(List<SeatEntity> entityList) {
        if ( entityList == null ) {
            return null;
        }

        List<SeatResponseDTO> list = new ArrayList<SeatResponseDTO>( entityList.size() );
        for ( SeatEntity seatEntity : entityList ) {
            list.add( entityToResponseDto( seatEntity ) );
        }
    ...
    @Override
    public SeatEntity updateEntityFromRequestDTO(SeatRequestDTO requestDTO, SeatEntity entity) {
        if ( requestDTO == null ) {
            return entity;
        }

        entity.setSeatNo( requestDTO.getSeatNo() );
        if ( requestDTO.getPatient() != null ) {
            if ( entity.getPatient() == null ) {
                entity.setPatient( new PatientEntity() );
            }
            patientRequestDTOToPatientEntity1( requestDTO.getPatient(), entity.getPatient() );
        }
        else {
    ...
    protected PatientEntity patientRequestDTOToPatientEntity(PatientRequestDTO patientRequestDTO) {
        ...
        patientEntity.setUser( userRequestDTOToUserEntity( patientRequestDTO.getUser() ) );
    ...
    protected void patientRequestDTOToPatientEntity1(PatientRequestDTO patientRequestDTO, PatientEntity mappingTarget) {
        if ( patientRequestDTO == null ) {
            return;
        }

        if ( patientRequestDTO.getUser() != null ) {
            if ( mappingTarget.getUser() == null ) {
                mappingTarget.setUser( new UserEntity() );
            }
            userRequestDTOToUserEntity1( patientRequestDTO.getUser(), mappingTarget.getUser() );
        }
        else {

...
public abstract class BaseService <Entity extends BaseEntity,
    ...
    @Modifying
    @Transactional
    public Boolean update(UUID uuid, RequestDTO requestDTO) {
    ...
    public Boolean getByUUID(UUID uuid) {
        Entity entity = getRepository().findByUuid(uuid).orElse(null);
        if (entity != null) {
            return getMapper().entityToDTO(entity);
        } else {
    ...
    public Page<DTO> getAllWithFilterAndPagination(BaseFilterRequestDTO baseFilterRequestDTO) {
        ...
        return (Page<DTO>) getMapper().pageEntityToPageDTO(entityPage);
    }

    public Object getAllByPageable(BaseFilterRequestDTO requestDto) {
    }
}
